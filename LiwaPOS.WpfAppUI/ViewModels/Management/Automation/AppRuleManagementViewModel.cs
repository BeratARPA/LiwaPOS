...
namespace LiwaPOS.WpfAppUI.ViewModels.Management.Automation
{
    public class AppRuleManagementViewModel : ViewModelBase
    {
        ...
        public AppRuleManagementViewModel(IAppRuleService appRuleService, IAppActionService appActionService, IRuleActionMapService ruleActionMapService, ICustomNotificationService customNotificationService)
        {
            ...
            return new List<(string PropertyName, Func<Task<bool>> ValidationFunction, string Message)>
    {
        (
            nameof(AppRuleName),
            async () => !string.IsNullOrEmpty(AppRuleName),
            string.Format(await TranslatorExtension.TranslateUI("NotEmpty_ph"), nameof(AppRuleName))
        ),
         (
            nameof(AppRuleType),
            async () => !string.IsNullOrEmpty(AppRuleType.ToString()),
            string.Format(await TranslatorExtension.TranslateUI("NotEmpty_ph"), nameof(AppRuleType))
        ),
         (
            nameof(SelectedActions),
            async () => !(SelectedActions.Count <= 0),
            string.Format(await TranslatorExtension.TranslateUI("NotEmpty_ph"), nameof(SelectedActions))
        ),
        (
            nameof(AppRuleName),
            async () => !(await _appRuleService.GetAllAppRulesAsNoTrackingAsync(x => x.Name == AppRuleName&& x.Id != AppRuleId)).Any(),
            string.Format(await TranslatorExtension.TranslateUI("AlreadyUse_ph"), nameof(AppRuleName))
        )
    };
        }
